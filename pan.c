...
short src_ln0 [] = {
	  0,  13,  15,  15,  15,  16,  16,  14, 
	 18,  18,  20,  20,  20,  21,  21,  19, 
	 24,  25,  26,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "Ex16.pml", 1, 18 },
...
{	int j, h = now._nr_pr;
	...
	case 0:	/* visit */
		...
		/* params: */
		/* locals: */
		((P0 *)pptr(h))->i = ((int)((P0 *)this)->_pid);
#ifdef VAR_RANGES
		logval("visit:i", ((P0 *)pptr(h))->i);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(int calling_pid)
{
	...
	{	int l_in;
		...
		now.count = 1;
#ifdef VAR_RANGES
		logval("goRight", now.goRight);
		logval("DOWN", now.DOWN);
		logval("RIGHT", now.RIGHT);
		logval("STOP", now.STOP);
...
{	/* int i; */
	...
	printf("	int    i:	%d\n", ((P1 *)pptr(pid))->i);
		break;
	case 0:
		printf("local vars proc %d (visit):\n", pid);
	printf("	int    i:	%d\n", ((P0 *)pptr(pid))->i);
		break;
	}
}
